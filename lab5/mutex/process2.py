...
from constMutex import ENTER, RELEASE, ALLOW, CHECK, ALIVE, WAIT
...
class Process:
    ...
    def __init__(self, chan):
        ...
        self.wait_time_base = 3
        self.cur_wait_time = 3
    ...
    def checkIn(self):
        ...
        self.logger.info("{}: Still alive. Sending ALIVE".format(self.__mapid()))
        self.channel.send_to(self.other_processes, (self.clock, self.process_id, ALIVE))            
        
    def hasToCheckIn(self):
        checks = [c for c in self.queue if c[2] == CHECK]
        return len(checks) != 0
            
    def __allowed_to_enter(self):
        # if self.hasToCheckIn():
        #     return False
    ...
    def __receive(self):
         # Pick up any message
        _receive = self.channel.receive_from(self.other_processes, self.cur_wait_time) 
        if _receive:
            ...
            if msg[2] == ENTER:
                ...
                if (self.suspect_crashed != ""):
                    ...
                    self.logger.info("{}: {} still alive. Increasing wait time. Continuing...".format(self.__mapid(), self.__mapid(sender)))
            ...
            #     self.channel.send_to([sender], msg)
                ...
                self.cur_wait_time += 1
                self.suspect_crashed = ""
                # del(self.queue[0]) # remove wait for ALIVE
    ...
    def handle_crashed_process(self):
        crashed = self.suspect_crashed #self.queue[0][1]
        self.cur_wait_time = self.wait_time_base
    ...
    def checkForCrash(self):
        ...
        self.logger.info("{}: {} potentially crashed. Sending check".format(self.__mapid(), self.__mapid(self.suspect_crashed)))
        # self.clock += 1
        msg = (self.clock, self.process_id, CHECK)
        # self.channel.send_to([self.suspect_crashed], msg)
        # self.queue.insert(0, (0, self.suspect_crashed, ALIVE)) # wait for ALIVE
    ...
    def run(self):
        while True:
            ...
            if len(self.all_processes) > 1 and \
                ...
                sleep_time = random.randint(0, self.cur_wait_time*1000)
                ...
                # print("still alive {}".format(self.__mapid()))
                time.sleep(sleep_time/1000)
